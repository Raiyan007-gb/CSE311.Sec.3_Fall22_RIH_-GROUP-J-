...
$Blood_Type = $Name = $Age = $Phone = $Location
=$Time =$Preferred_Date ="";
$Blood_Type_err = $Name_err = $Age_err = $Phone_err = $Location_err
= $Time_err = $Preferred_Date_err ="";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($Name_err) && empty($Age_err) && empty($Phone_err)&& empty($Location_err)&& empty($Time_err)
    && empty($Blood_Type_err)&& empty($Preferred_Date_err)){
        // Prepare an update statement
        $sql = "INSERT INTO blood_request_user (Blood_Type, User_ID,Name,Location,Phone,Preferred_Date
        ,Time,Age) VALUES ( ?,?,?, 
        ?, ?,?,?, ?)";
        
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sisssssi", $param_Blood_Type,$param_id,$param_Name,$param_Location,$param_Phone,
        $param_Preferred_Date,$param_Time,$param_Age);
                        ...
                        // Attempt to execute the prepared statement
                            if(mysqli_stmt_execute($stmt)){
                                ...
                                exit();
                                  ...
                                  echo '<h1><center><b>" You have already requested,please <a href="delete_blood_request_user.php?link=$a">DELETE</a> it to continue </b> <br>
...
<body>
    <div class="wrapper">
        ...
        <h3>USER ID <?php echo htmlspecialchars($_SESSION["User_ID"]); ?></h3>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
            <br>
            <div class="form-group <?php echo (!empty($Blood_Type_err)) ? 'has-error' : ''; ?>">
                 <label for="Blood_Type"><h4><b>Choose Blood Type:</b></h4></label>
